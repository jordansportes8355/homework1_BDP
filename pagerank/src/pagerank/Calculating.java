...
public class Calculating extends Configured implements Tool {
   ...
   public int run(String[] args) throws Exception {
      ...
      FileInputFormat.addInputPath(job, new Path("output1PageRank"));
      FileOutputFormat.setOutputPath(job, new Path("output2PageRank"));
      FileSystem hdfs = FileSystem.get(getConf());
 	  if (hdfs.exists(new Path("output2PageRank")))
 	      hdfs.delete(new Path("output2PageRank"), true);
   ...
   public static class Reduce extends Reducer<Text, Text, Text, Text> {
      ...
      public void reduce(Text key, Iterable<Text> values, Context context)
    	  ...
    	  context.write(key, new Text(""+pageRankScore));
    	  topPages.add(new PageAndRank(pageRankScore,key.toString()));
    	   if (topPages.size() > 10) {
    		  ...
    		  for(Text val : values){
    			  ...
    			  context.write(new Text(res[0]), new Text(""+pageRankScore));
